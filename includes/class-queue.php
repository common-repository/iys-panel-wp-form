...
class IM4WP_Queue {
	...
	/**
	 * IM4WP_Ecommerce_Queue constructor.
	 *
	...
	protected function load() {
		...
		if ( ! is_array( $jobs ) ) {
			...
			foreach ( $jobs as $i => $obj ) {
				...
				// make sure each job is instance of IM4WP_Queue_Job
				if ( $obj instanceof IM4WP_Queue_Job ) {
					$job = $obj;
				} else {
					$job     = new IM4WP_Queue_Job( $obj->data );
					$job->id = $obj->id;
				}
	 ...
	 *
	 * @return IM4WP_Queue_Job[] Array of jobs
	 */
	public function all() {
		$this->load();
		return $this->jobs;
	...
	/**
	 * @param IM4WP_Queue_Job $job
	 */
	public function delete( IM4WP_Queue_Job $job ) {
		$this->load();
	 ...
	 * @param IM4WP_Queue_Job $job
	 */
	public function reschedule( IM4WP_Queue_Job $job ) {
		$this->load();
