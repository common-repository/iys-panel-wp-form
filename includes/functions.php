 ...
 */
function im4wp( $service = null ) {
	static $im4wp;

	if ( ! $im4wp ) {
		$im4wp = new IM4WP_Container();
	}

	if ( $service ) {
		return $im4wp->get( $service );
	}
...
/**
 * Gets the İYS Panel WP Form options from the database
 ...
 * @return array
 */
function im4wp_get_options() {
	$defaults = require IM4WP_PLUGIN_DIRDIR . '/config/default-settings.php';
	$options  = (array) get_option( 'im4wp', array() );
	$options  = array_merge( $defaults, $options );

	/**
	 * Filters the İYS Panel WP Form settings (general).
	 *
	 * @param array $options
	 */
	return apply_filters( 'im4wp_settings', $options );
}

/**
 * @return array
 */
function im4wp_get_settings() {
	return im4wp_get_options();
}
 ...
 * @return string
 */
function im4wp_get_api_key() {
	// try to get from constant
	if ( defined( 'IM4WP_API_KEY' ) && constant( 'IM4WP_API_KEY' ) !== '' ) {
		return IM4WP_API_KEY;
	}
...
/**
 * Gets the İYS Panel WP Form API class (v3) and injects it with the API key
 *
 * @since 4.0
 * @access public
 *
 * @return IM4WP_API_V1
 */
function im4wp_get_api_v1() {
	$api_key  = im4wp_get_api_key();
	$instance = new IM4WP_API_V1( $api_key );
	return $instance;
}

/**
 * Gets the İYS Panel WP Form API class and injects it with the API key
 *
 * @deprecated 4.0
 * @use im4wp_get_api_v1
 *
 * @since 1.0
 * @access public
 *
 * @return IM4WP_API
 */
function im4wp_get_api() {
	_deprecated_function( __FUNCTION__, '4.0', 'im4wp_get_api_v1' );
	$api_key  = im4wp_get_api_key();
	$instance = new IM4WP_API( $api_key );
	return $instance;
}
 ...
 */
function im4wp_get_debug_log() {
	$opts = im4wp_get_options();

	// get default log file location
	$upload_dir = wp_upload_dir( null, false );
	$file       = $upload_dir['basedir'] . '/iys-panel-wp-form/debug-log.php';
	$default_file = $file;

	/**
	 * Filters the log file to write to.
	 *
	 * @param string $file The log file location. Default: /wp-content/uploads/iys-panel-wp-form/im4wp-debug.log
	 */
	$file = apply_filters( 'im4wp_debug_log_file', $file );
	 ...
	 */
	$level = apply_filters( 'im4wp_debug_log_level', $opts['debug_log_level'] );

	return new IM4WP_Debug_Log( $file, $level );
}
 ...
 * @return string
 */
function im4wp_plugin_url( $path ) {
	static $base = null;
	if ( $base === null ) {
		$base = plugins_url( '/', IM4WP_PLUGIN_DIRFILE );
	}
 ...
 * @return string
 */
function im4wp_get_request_url() {
	global $wp;
 ...
 * @return string
 */
function im4wp_get_request_path() {
...
* @return string|null
*/
function im4wp_get_request_ip_address() {
 ...
 * @return mixed
 */
function im4wp_sanitize_deep( $value ) {
	if ( is_scalar( $value ) ) {
		...
		$value = array_map( 'im4wp_sanitize_deep', $value );
	} elseif ( is_object( $value ) ) {
		$vars = get_object_vars( $value );
		foreach ( $vars as $key => $data ) {
			$value->{$key} = im4wp_sanitize_deep( $data );
		}
	}
 ...
 * @return array
 */
function im4wp_update_groupings_data( $data = array() ) {
	...
	$map = get_option( 'im4wp_groupings_map', array() );
 ...
 * @return array
 */
function im4wp_add_name_data( $data = array() ) {
...
function im4wp_get_email_type() {
	 ...
	 */
	$email_type = (string) apply_filters( 'im4wp_email_type', $email_type );
 ...
 * @return string
 */
function im4wp_obfuscate_string( $string ) {
 ...
 * @ignore
 */
function im4wp_obfuscate_email_addresses_callback( $m ) {
 ...
 * @return string
 */
function im4wp_obfuscate_email_addresses( $string ) {
	return preg_replace_callback( '/([\w\.]{1,4})([\w\.]*)\@(\w{1,2})(\w*)\.(\w+)/', 'im4wp_obfuscate_email_addresses_callback', $string );
}

/**
 * Refreshes İYS Panel lists. This can take a while if the connected İYS Panel account has many lists.
 *
 * @return void
 */
function im4wp_refresh_iyspanel_lists() {
	$iyspanel = new IM4WP_MailChimp();
	$iyspanel->refresh_lists();
}
...
* @return mixed
*/
function im4wp_array_get( $array, $key, $default = null ) {
	if ( is_null( $key ) ) {
		return $array;
	}
