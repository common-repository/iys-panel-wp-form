 ...
 *
 * To get a form instance, use `im4wp_get_form( $id );` where `$id` is the post ID.
 *
 * @access public
 * @since 3.0
 */
class IM4WP_Form {
	...
	public static function throw_not_found_exception( $post_id ) {
		$message = sprintf( __( 'There is no form with ID %d, perhaps it was deleted?', 'iys-panel-wp-form' ), $post_id );
	...
	public static function get_instance( $post = 0 ) {
		if ( $post instanceof WP_Post ) {
			...
			if ( $post_id === 0 ) {
				$post_id = (int) get_option( 'im4wp_default_form_id', 0 );
			}
		}
		...
		// check post object
		if ( ! $post instanceof WP_Post || $post->post_type !== 'im4wp-form' ) {
			self::throw_not_found_exception( $post_id );
		}

		// get all post meta in single call for performance
		$post_meta = (array) get_post_meta( $post_id );
		$form      = new IM4WP_Form( $post_id, $post, $post_meta );
    ...
    /**
	 * @var bool Was this form is 2FA
	 */
	public $is_2fa = false;
    public $two_fa_sms_key = '';
	 ...
	 *
	 * @see IM4WP_Form_Element::get_response_html()
	 *
	 * @return string
	 */
	 ...
	 * @param array $config
	 * @return IM4WP_Form_element
	 */
	public function get_element( $element_id = 'im4wp-form', array $config = array() ) {
		return new IM4WP_Form_Element( $this, $element_id, $config );
	}

	/**
	 * Get HTML string for this form.
	 *
	 * If you want to output a form, use `im4wp_show_form` instead as it.
	 *
	 * @param string $element_id
	 * @param array $config
	 *
	 * @return string
	 */
	public function get_html( $element_id = 'im4wp-form', array $config = array() ) {
	...
	protected function load_settings( array $post_meta = array() ) {
		$form             = $this;
		$default_settings = include IM4WP_PLUGIN_DIRDIR . '/config/default-form-settings.php';

		// start with defaults
		$settings = $default_settings;

		// get custom settings from meta
		if ( ! empty( $post_meta['_im4wp_settings'] ) ) {
			$meta = $post_meta['_im4wp_settings'][0];
		 ...
		 */
		$settings = (array) apply_filters( 'im4wp_form_settings', $settings, $form );
	...
	protected function load_messages( array $post_meta = array() ) {
		...
		$default_messages = include IM4WP_PLUGIN_DIRDIR . '/config/default-form-messages.php';
		 ...
		 * @param IM4WP_Form $form
		 */
		$messages = (array) apply_filters( 'im4wp_form_messages', $messages, $form );
	...
	public function add_notice( $text, $type = 'notice' ) {
		$this->notices[] = new IM4WP_Form_Notice( $text, $type );
	}
	...
	public function __toString() {
		return im4wp_show_form( $this->ID, array(), false );
	}
	...
	public function get_redirect_url() {
		 ...
		 * @param IM4WP_Form $form
		 */
		$url = (string) apply_filters( 'im4wp_form_redirect_url', $url, $form );
		return $url;
	}
	...
	public function validate() {
		...
		if ( ! isset( $this->raw_data['_im4wp_timestamp'] ) || $this->raw_data['_im4wp_timestamp'] > ( time() - 2 ) ) {
			$errors[] = 'spam';
		} elseif ( ! isset( $this->raw_data['_im4wp_honeypot'] ) || ! empty( $this->raw_data['_im4wp_honeypot'] ) ) {
			$errors[] = 'spam';
		}
		...
		if ( empty( $errors ) ) {
			...
			foreach ( $this->get_required_fields() as $field ) {
				$value = im4wp_array_get( $this->data, $field );
		 ...
		 * @param IM4WP_Form $form
		 */
		$errors = (array) apply_filters( 'im4wp_form_errors', $errors, $form );

		/**
		 * @ignore
		 * @deprecated 3.0 Use `im4wp_form_errors` instead
		 */
		$form_validity = apply_filters( 'im4wp_valid_form_request', true, $this->data );
	...
	public function handle_request( array $data ) {
        ...
        /* if (array_key_exists('GSM', $data) && ! array_key_exists('EMAIL', $data)) {
         *     $this->is_2fa = true;
         * } */

        if (array_key_exists('_im4wp_form_2fa_key', $data) || array_key_exists('resend2fa', $data)) {
            $this->is_2fa = true;
        }
		...
		$map    = array(
			'_im4wp_lists'           => 'lists',
			'_im4wp_action'          => 'action',
			'_im4wp_form_element_id' => 'element_id',
			'_im4wp_email_type'      => 'email_type',
		);
	...
	protected function parse_request_data( array $data ) {
		 ...
		 * @param IM4WP_Form $form The form instance.
		 */
		$ignored_field_names = apply_filters( 'im4wp_form_ignored_field_names', $ignored_field_names, $form );
	...
	public function set_config( array $config ) {
		...
		if ( ! in_array( $this->config['action'], array( 'subscribe', 'unsubscribe', '2fa' ), true ) ) {
	...
	/**
	 * Get İYS Panel lists this form subscribes to
	 *
	 * @return array
	 */
	public function get_lists() {
		...
		/**
		 * Filters İYS Panel lists new subscribers should be added to.
		 *
		 * @param array $lists
		 */
		$lists = (array) apply_filters( 'im4wp_lists', $lists );

		/**
		 * Filters İYS Panel lists new subscribers coming from this form should be added to.
		 *
		 * @param array $lists
		 * @param IM4WP_Form $form
		 */
		$lists = (array) apply_filters( 'im4wp_form_lists', $lists, $form );
	...
	public function get_data() {
		 ...
		 * @param IM4WP_Form $form
		 */
		$data = apply_filters( 'im4wp_form_data', $data, $form );
	...
	public function get_required_fields() {
		...
		// workaround for #261 (https://github.com/iyspanel/iyspanel-for-wordpress/issues/261)
		...
		// fix uppercased subkeys, see https://github.com/iyspanel/iyspanel-for-wordpress/issues/516
		 ...
		 *
		 * - All fields which are required for the selected İYS Panel lists
		 * - All fields in the form with a `required` attribute.
		 *
		 * @param array $required_fields
		 * @param IM4WP_Form $form
		 */
		$required_fields = (array) apply_filters( 'im4wp_form_required_fields', $required_fields, $form );

		return $required_fields;
	}

	/**
	 * Get "email_type" setting for new İYS Panel subscribers added by this form.
	 *
	 * @return string
	 */
	public function get_email_type() {
		...
		if ( empty( $email_type ) ) {
			$email_type = im4wp_get_email_type();
		}
	...
	public function get_message( $key ) {
		...
		if ( $key === 'no_lists_selected' && current_user_can( 'manage_options' ) ) {
			$message .= sprintf( ' (<a href="%s">%s</a>)', im4wp_get_edit_form_url( $this->ID, 'settings' ), 'edit form settings' );
		}
