...
/**
 * Class IM4WP_Form_Output_Manager
 *
 * @ignore
 * @access private
 */
class IM4WP_Form_Output_Manager {
	...
	public function add_hooks() {
		...
		add_filter( 'im4wp_form_content', 'do_shortcode' );
		add_action( 'init', array( $this, 'register_shortcode' ) );
	}

	/**
	 * Registers the [im4wp_form] shortcode
	 */
	...
	public function output_form( $id = 0, $config = array(), $echo = true ) {
		try {
			$form = im4wp_get_form( $id );
		} catch ( Exception $e ) {
			if ( current_user_can( 'manage_options' ) ) {
				return sprintf( '<strong>İYS Panel WP Form error:</strong> %s', $e->getMessage() );
			}
		...
		if ( empty( $config['element_id'] ) ) {
			$config['element_id'] = 'im4wp-form-' . $this->count;
		}
		...
		try {
			 ...
			 */
			do_action( 'im4wp_output_form', $form );
