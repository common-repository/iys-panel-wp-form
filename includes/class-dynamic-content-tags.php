...
/**
 * Class IM4WP_Dynamic_Content_Tags
 *
 * @access private
 * @ignore
 */
abstract class IM4WP_Dynamic_Content_Tags {
	...
	protected function register() {
		...
		$this->tags['cookie'] = array(
			'description' => sprintf( __( 'Data from a cookie.', 'iys-panel-wp-form' ) ),
			'callback'    => array( $this, 'get_cookie' ),
			'example'     => "cookie name='my_cookie' default='Default Value'",
		);

		$this->tags['email'] = array(
			'description' => __( 'The email address of the current visitor (if known).', 'iys-panel-wp-form' ),
			'callback'    => array( $this, 'get_email' ),
		);

		$this->tags['current_url'] = array(
			'description' => __( 'The URL of the page.', 'iys-panel-wp-form' ),
			'callback'    => 'im4wp_get_request_url',
		);

		$this->tags['current_path'] = array(
			'description' => __( 'The path of the page.', 'iys-panel-wp-form' ),
			'callback'    => 'im4wp_get_request_path',
		);

		$this->tags['date'] = array(
			'description' => sprintf( __( 'The current date. Example: %s.', 'iys-panel-wp-form' ), '<strong>' . gmdate( 'Y/m/d', time() + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) ) . '</strong>' ),
			'replacement' => gmdate( 'Y/m/d', time() + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) ),
		);

		$this->tags['time'] = array(
			'description' => sprintf( __( 'The current time. Example: %s.', 'iys-panel-wp-form' ), '<strong>' . gmdate( 'H:i:s', time() + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) ) . '</strong>' ),
			'replacement' => gmdate( 'H:i:s', time() + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) ),
		);

		$this->tags['language'] = array(
			'description' => sprintf( __( 'The site\'s language. Example: %s.', 'iys-panel-wp-form' ), '<strong>' . get_locale() . '</strong>' ),
			'callback'    => 'get_locale',
		);

		$this->tags['ip'] = array(
			'description' => sprintf( __( 'The visitor\'s IP address. Example: %s.', 'iys-panel-wp-form' ), '<strong>' . im4wp_get_request_ip_address() . '</strong>' ),
			'callback'    => 'im4wp_get_request_ip_address',
		);

		$this->tags['user'] = array(
			'description' => sprintf( __( 'The property of the currently logged-in user.', 'iys-panel-wp-form' ) ),
			'callback'    => array( $this, 'get_user_property' ),
			'example'     => "user property='user_email'",
		);

		$this->tags['post'] = array(
			'description' => sprintf( __( 'Property of the current page or post.', 'iys-panel-wp-form' ) ),
