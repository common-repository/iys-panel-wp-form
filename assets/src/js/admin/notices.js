...
function render () {
  ...
  let container = document.querySelector('.im4wp-notices')
  if (!container) {
    container = document.createElement('div')
    container.className = 'im4wp-notices'
...
const groupingsNotice = function () {
  const text = 'Your form contains deprecated <code>GROUPINGS</code> fields. <br /><br />Please remove these fields from your form and then re-add them through the available field buttons to make sure your data is getting through to İYS Panel correctly.'
...
const requiredFieldsNotice = function () {
  ...
  let text = '<strong>Heads up!</strong> Your form is missing list fields that are required in İYS Panel. Either add these fields to your form or mark them as optional in İYS Panel.'
...
const iyspanelListsNotice = function () {
  const text = '<strong>Heads up!</strong> You have not yet selected a İYS Panel list to subscribe people to. Please select at least one list from the <a href="javascript:void(0)" data-tab="settings" class="tab-link">settings tab</a>.'
...
document.body.addEventListener('change', iyspanelListsNotice)
