...
function showDetails (evt) {
  ...
  if (next.style.display === 'none') {
    m.request({
      method: 'GET',
      url: ajaxurl + '?action=im4wp_get_list_details&ids=' + listID
    }).then(details => {
      ...
      m('table.striped.widefat', [
        ...
        m('tbody', data.interest_categories.map(f => (
          m('tr', [
            ...
            m('td', f.type),
            m('td', [
              m('div.im4wp-row', { style: 'margin-bottom: 4px;' }, [
                m('div.im4wp-col.im4wp-col-3', [
              ...
              Object.keys(f.interests).map((id) => (
                m('div.im4wp-row.im4wp-margin-s', [
                  m('div.im4wp-col.im4wp-col-3', f.interests[id]),
                  m('div.im4wp-col.im4wp-col-3', [
...
const table = document.getElementById('im4wp-iyspanel-lists-overview')
if (table) {
  table.addEventListener('click', (evt) => {
    if (!evt.target.matches('.im4wp-iyspanel-list')) {
      return
    }
